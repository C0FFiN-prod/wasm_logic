...
function updateIcons() {
    ...
    const drawSvg = (path: Path2D, fillColor: string, shadow: boolean) => {
        ...
        ctx.scale(0.65, 0.65);
        ctx.shadowColor = shadow ? fillColor : "transparent";
    ...
    ctx.shadowOffsetX = 0;
    ctx.shadowOffsetY = 0;
    for (const [icon, v] of pathMap.entries()) {
        const activePath = activePathMap.get(icon);
        const pathX0 = new Path2D(v);
        const pathX1 = activePath === undefined ? pathX0 : new Path2D(activePath);
        drawSvg(pathX0, "#FFFFFF80", false);
        drawSvg(pathX1, "#FFFFFFFF", true);
        drawSvg(pathX0, "#00000080", false);
        drawSvg(pathX1, "#000000FF", true);
        ctx.translate(-4 * gridSize, gridSize);
        iconMap.set(icon, row);
        ++row;
    }
    ctx.restore();
